...
public class JwtServiceImpl implements JwtService{
    ...
    @Override
    public Jws<Claims> getClaims(final String token){//유효한 토큰인지 확인
        try {
    ...
    public String generateRefreshToken(final String email){//
        ...
        return Jwts.builder().setHeaderParam(Header.JWT_TYPE, JwtType.REFRESH)
                .setSubject(email)
                .setIssuedAt(new Date(System.currentTimeMillis()))
    ...
    @Override
    public String extractToken(final String token){//Bearer 쓰는 이유 - 토큰마다 인증할려고
    ...
    public Authentication getAuthentication(String token) {
        ...
        UsersEntity users = usersRepository.findByEmailLogIn(claims.getBody().getSubject()).orElseThrow(
